...
export const executeApiSchema = z.object({
  ...
  generateCode: z
    ...
    .optional()
    .default(false)
    .describe("Generate code snippet if true"),
  region: z
    .string()
    .optional()
    .default("us-east-1")
...
export class ExecuteApiTool {
  ...
  async execute(params: ExecuteApiParams): Promise<string> {
    logger.debug(
      "Executing execute-sp-api tool with params:",
    ...
    try {
      ...
      if (!endpoint) {
        return this.formatError(
          `Endpoint '${params.endpoint}' not found in the catalog`,
        );
      }
  ...
  private findEndpoint(endpointId: string): ApiEndpoint | undefined {
    ...
    for (const priorityCategoryName of priorityCategories) {
      const category = this.catalog.categories.find(
        (c) => c.name === priorityCategoryName,
      );
      if (category) {
        const endpoint = category.endpoints.find(
          (e) => e.originalOperationId === endpointId,
        );
        if (endpoint) {
          return endpoint;
        }
      ...
      for (const param of queryParams) {
        if (parameters[param.name] !== undefined) {
          ...
          if (Array.isArray(parameters[param.name])) {
            ...
            queryParts.push(
              `${encodeURIComponent(param.name)}=${encodeURIComponent(parameters[param.name])}`,
            );
          }
        }
      }
      ...
      for (const [key, value] of Object.entries(response)) {
        if (Array.isArray(value)) {
          ...
          if (value.length > 0 && typeof value[0] === "object") {
            ...
            const sampleValues = sampleProps.map(
              (prop) =>
                `${prop}: ${JSON.stringify(sampleItem[prop]).substring(0, 50)}`,
            );
            highlights.push(
              `Sample item properties: ${sampleValues.join(", ")}`,
            );
          }
        }
      }
      ...
      if (endpoint.id.includes("Inventory")) {
        if (response.payload?.inventorySummaries) {
          ...
          if (itemsWithInventory > 0) {
            highlights.push(
              `Total fulfillable quantity: ${totalFulfillable} units across ${itemsWithInventory} SKUs`,
            );
          }
        }
      }
  ...
  private generateInsights(
    ...
    if (!success || !response) {
      return [
        "The request was not successful. Please check the error details.",
      ];
    }

    const insights: string[] = ["The request was successful."];
    ...
    if (endpoint.id.includes("get") || endpoint.id.includes("list")) {
      if (response.nextToken || response.payload?.nextToken) {
        insights.push(
          "There are more results available. Use the nextToken parameter to retrieve the next page.",
        );
      }
      ...
      if (itemCount > 0) {
        ...
        insights.push(
          "No items were retrieved. This could be due to filtering or because there are no items available.",
        );
      }
    }
    ...
    if (
      endpoint.id.includes("Inventory") &&
      response.payload?.inventorySummaries
    ) {
      const summaries = response.payload.inventorySummaries;
      ...
      if (skusWithUnfulfillable > 0) {
        insights.push(
          `Found ${totalUnfulfillable} unfulfillable units across ${skusWithUnfulfillable} SKUs. Consider checking these items.`,
        );
      }
      ...
      for (const summary of summaries) {
        const reserved =
          summary.inventoryDetails?.reservedQuantity?.totalReservedQuantity ||
          0;
        if (reserved > 0) {
          skusWithReserved++;
        }
      ...
      if (skusWithReserved > 0) {
        insights.push(
          `${skusWithReserved} SKUs have units reserved for customer orders or other processes.`,
        );
      }
    }
  ...
  private suggestNextSteps(endpoint: ApiEndpoint, success: boolean): string[] {
    ...
    if (endpoint.id.includes("get") || endpoint.id.includes("list")) {
      ...
      if (endpoint.id.includes("list")) {
        nextSteps.push(
          "Consider retrieving additional pages if nextToken is present.",
        );
        nextSteps.push(
          "Filter results to narrow down to specific items if needed.",
        );
      }
    } else if (endpoint.id.includes("create") || endpoint.id.includes("add")) {
      nextSteps.push(
        "Use the ID of the created resource for subsequent operations.",
      );
    ...
    if (endpoint.id.includes("Inventory")) {
      nextSteps.push(
        "Consider creating an inbound shipment for low-stock items.",
      );
      nextSteps.push(
        "Check for unfulfillable inventory and determine the cause.",
      );
    }
  ...
  private generateErrorRecommendations(
    ...
    switch (statusCode) {
      ...
      case 403:
        ...
        recommendations.push(
          "Verify that you have the necessary roles and permissions.",
        );
        recommendations.push(
          "Check if you are using the correct marketplace ID.",
        );
        break;
      case 404:
      ...
      case 429:
        recommendations.push(
          "You have exceeded the rate limits for this operation.",
        );
      ...
      case 503:
        ...
        recommendations.push(
          "Check the Amazon Seller Central status page for service outages.",
        );
        break;
      default:
        recommendations.push("Review the error details for more information.");
        recommendations.push(
          "Check the SP-API documentation for this endpoint.",
        );
    }
    ...
    if (errorResponse?.errors) {
      for (const error of errorResponse.errors) {
        if (error.code === "InvalidInput") {
          recommendations.push(
            "One or more input parameters are invalid. Check the error details.",
          );
        } else if (error.code === "AccessDenied") {
          recommendations.push(
            "You do not have access to this resource. Verify your permissions.",
          );
        } else if (error.code === "QuotaExceeded") {
          recommendations.push(
            "You have exceeded your quota for this operation. Try again later.",
          );
        } else if (error.code === "ResourceNotFound") {
          recommendations.push(
            "The requested resource does not exist. Verify your identifiers.",
          );
        }
      }
    }
