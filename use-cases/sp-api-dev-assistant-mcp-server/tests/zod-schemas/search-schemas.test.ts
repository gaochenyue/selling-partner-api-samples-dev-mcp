  ...
  describe('searchSchema', () => {
    ...
    describe('property-based tests', () => {
      it('should accept any non-empty string query with valid top_k', () => {
        fc.assert(
          fc.property(fc.string({ minLength: 1 }), fc.integer({ min: 1, max: 1000 }), (query, top_k) => {
      ...
      it('should reject non-positive top_k values', () => {
        fc.assert(
          fc.property(fc.string({ minLength: 1 }), fc.integer({ min: -1000, max: 0 }), (query, top_k) => {
            const result = searchSchema.safeParse({ query, top_k });
            expect(result.success).toBe(false);
          }),
          { numRuns: 100 },
      ...
      it('should reject non-integer top_k values', () => {
        fc.assert(
          fc.property(
            ...
            fc.double({ min: 0.01, max: 1000, noNaN: true }).filter((n) => !Number.isInteger(n)),
            (query, top_k) => {
              const result = searchSchema.safeParse({ query, top_k });
              expect(result.success).toBe(false);
            },
    ...
    describe('valid inputs', () => {
      it('should accept query with top_k', () => {
        const result = searchSchema.safeParse({ query: 'How do I use the Orders API?', top_k: 10 });
        expect(result.success).toBe(true);
        if (result.success) {
