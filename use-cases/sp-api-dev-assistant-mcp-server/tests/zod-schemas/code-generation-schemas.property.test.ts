...
describe('Code Generation Schemas - Property-Based Tests', () => {
  ...
  describe('Property 2: Language Enum Validation Consistency', () => {
    ...
    it('should reject invalid language values across all schemas', () => {
      fc.assert(
        fc.property(
          fc.string().filter((s) => !validLanguages.includes(s) && s.length > 0),
          (invalidLanguage) => {
            // Test getBasicUsageSchema
      ...
      fc.assert(
        fc.property(
          ...
          (items, page, pageSize, totalItems) => {
            ...
            expect(paginatedResponse.pagination).toHaveProperty('hasPreviousPage');

            // Validate pagination logic
            expect(paginatedResponse.pagination.page).toBeGreaterThanOrEqual(1);
            expect(paginatedResponse.pagination.pageSize).toBeGreaterThanOrEqual(1);
            expect(paginatedResponse.pagination.pageSize).toBeLessThanOrEqual(100);
            expect(paginatedResponse.pagination.totalItems).toBeGreaterThanOrEqual(0);
            expect(paginatedResponse.pagination.totalPages).toBeGreaterThanOrEqual(0);
  ...
  describe('Property 5: Invalid Path Error Handling', () => {
    it('should validate that empty paths are rejected', () => {
      fc.assert(
        fc.property(fc.constantFrom('python', 'java', 'javascript', 'php', 'csharp'), (language) => {
...
const validLanguages = ['python', 'java', 'javascript', 'php', 'csharp'] as const;
...
describe('Master Code Generation Schema - Property-Based Tests', () => {
  ...
  describe('Property 2: Action enum validation', () => {
    ...
    it('should reject invalid action values', () => {
      fc.assert(
        fc.property(
          fc.string().filter((s) => !(validActions as readonly string[]).includes(s)),
          (invalidAction) => {
            const result = masterCodeGenerationSchema.safeParse({ action: invalidAction });
            expect(result.success).toBe(false);
          },
        ),
        { numRuns: 100 },
  ...
  describe('Property 3: Per-action parameter schema validation', () => {
    it('get_workflow_guide accepts with optional step and rejects invalid step', () => {
      fc.assert(
        fc.property(
          fc.option(fc.constantFrom('basic-usage', 'categories', 'operations', 'models', 'all'), { nil: undefined }),
          (step) => {
            const input: Record<string, unknown> = { action: 'get_workflow_guide' };
      ...
      fc.assert(
        fc.property(
          ...
          (repositoryUrl, targetPath) => {
            ...
            if (repositoryUrl !== undefined) input.repositoryUrl = repositoryUrl;
    ...
    it('get_basic_usage accepts with language provided', () => {
      fc.assert(
        fc.property(fc.constantFrom(...validLanguages), (language) => {
          const valid = masterCodeGenerationSchema.safeParse({ action: 'get_basic_usage', language });
    ...
    it('get_categories accepts with language provided', () => {
      fc.assert(
        fc.property(fc.constantFrom(...validLanguages), (language) => {
          const valid = masterCodeGenerationSchema.safeParse({ action: 'get_categories', language });
          expect(valid.success).toBe(true);
        }),
        { numRuns: 100 },
      );
    ...
    it('get_operations accepts with language and filePath provided', () => {
      fc.assert(
        fc.property(fc.constantFrom(...validLanguages), fc.string({ minLength: 1 }), (language, filePath) => {
          const valid = masterCodeGenerationSchema.safeParse({ action: 'get_operations', language, filePath });
          expect(valid.success).toBe(true);
        }),
        { numRuns: 100 },
      );
    ...
    it('get_models accepts with language and directoryPath provided', () => {
      fc.assert(
        fc.property(fc.constantFrom(...validLanguages), fc.string({ minLength: 1 }), (language, directoryPath) => {
          const valid = masterCodeGenerationSchema.safeParse({ action: 'get_models', language, directoryPath });
          expect(valid.success).toBe(true);
        }),
        { numRuns: 100 },
      );
  ...
  describe('Property 5: Pagination bounds validation', () => {
    it('should reject invalid page values (< 1) for get_operations and get_models', () => {
      fc.assert(
        fc.property(fc.integer({ max: 0 }), fc.constantFrom(...validLanguages), (invalidPage, language) => {
  ...
  describe('Property 6: Schema describe annotation completeness', () => {
    it('every field in the flat master schema has a non-empty description', () => {
      const shape = masterCodeGenerationSchema.shape as Record<string, { description?: string }>;
