  ...
  describe('optimizationSchema', () => {
    describe('valid inputs', () => {
      ...
      it('should accept source_code only', () => {
        const result = optimizationSchema.safeParse({ source_code: 'const x = 1;' });
        expect(result.success).toBe(true);
        if (result.success) {
      ...
      it('should accept all fields provided', () => {
        ...
        if (result.success) {
          ...
          expect(result.data.optimization_goals).toEqual(['error_handling', 'batching']);
      ...
      it('should accept each valid optimization goal individually', () => {
        ...
        for (const goal of goals) {
          const result = optimizationSchema.safeParse({ optimization_goals: [goal] });
          expect(result.success).toBe(true);
        }
      });
    ...
    describe('invalid inputs', () => {
      ...
      it('should reject invalid optimization goal', () => {
        const result = optimizationSchema.safeParse({ optimization_goals: ['invalid_goal'] });
        expect(result.success).toBe(false);
        if (!result.success) {
      ...
      it('should reject non-array optimization_goals', () => {
        const result = optimizationSchema.safeParse({ optimization_goals: 'batching' });
        expect(result.success).toBe(false);
        if (!result.success) {
