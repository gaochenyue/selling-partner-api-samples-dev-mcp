...
import { isOperation, Operation } from '../../src/tools/code-generation-tools/models/operation.js';
...
> = {
  ...
  listInboundShipments: { requestsPerSecond: 1, burst: 1, intervalInSeconds: 1 },
...
const APPLICATIONS_API_EXPECTED = {
  rotateApplicationClientSecret: { requestsPerMinute: 1, burst: 1, intervalInSeconds: 60 },
};
...
function expectRateLimitFromYaml(
  ...
  opName: string,
  expected: { requestsPerSecond?: number; requestsPerMinute?: number; burst: number; intervalInSeconds: number },
) {
  expect(op, `Operation "${opName}" should be found`).toBeDefined();
  expect(op!.rateLimit, `Operation "${opName}" should have a rateLimit from YAML`).not.toBeNull();
...
describe.skipIf(!SDK_AVAILABLE)('JavaParser integration - YAML rate limit lookup', () => {
  ...
  it('should parse AwdApi operations with rate limits from YAML (2-element entries)', async () => {
    ...
    expectRateLimitFromYaml(op, 'getInboundShipment', AWD_API_EXPECTED_RATE_LIMITS.getInboundShipment);
  });
  ...
  it('should produce operations that pass isOperation type guard', async () => {
    ...
    for (const op of operations) {
      expect(isOperation(op), `Java operation "${op.name}" should pass isOperation`).toBe(true);
    }
  });
});
...
describe.skipIf(!SDK_AVAILABLE)('JavaScriptParser integration - YAML rate limit lookup', () => {
  const parser = new JavaScriptParser(REPO_PATH);
  const awdApiPath = path.join(REPO_PATH, 'javascript/sdk/src/awd_v2024_05_09/api/AwdApi.js');
  const applicationsApiPath = path.join(
    REPO_PATH,
  ...
  it('should produce operations that pass isOperation type guard', async () => {
    const operations = await parser.parseOperations(awdApiPath);
    for (const op of operations) {
      expect(isOperation(op), `JS operation "${op.name}" should pass isOperation`).toBe(true);
    }
  });
});
...
describe.skipIf(!SDK_AVAILABLE)('CSharpParser integration - YAML rate limit lookup', () => {
  const parser = new CSharpParser(REPO_PATH);
  const awdApiPath = path.join(REPO_PATH, 'csharp/sdk/src/software.amzn.spapi/Api.awd.v2024_05_09/AwdApi.cs');
  const applicationsApiPath = path.join(
    REPO_PATH,
  ...
  it('should produce operations that pass isOperation type guard (including null rateLimits)', async () => {
    const operations = await parser.parseOperations(awdApiPath);
    for (const op of operations) {
      expect(isOperation(op), `CSharp operation "${op.name}" should pass isOperation`).toBe(true);
    }
  });
});
...
describe.skipIf(!SDK_AVAILABLE)('PHPParser integration - YAML rate limit lookup', () => {
  const parser = new PHPParser(REPO_PATH);
  const awdApiPath = path.join(REPO_PATH, 'php/sdk/lib/Api/awd/v2024_05_09/AwdApi.php');
  const applicationsApiPath = path.join(REPO_PATH, 'php/sdk/lib/Api/applications/v2023_11_30/ApplicationsApi.php');
  ...
  it('should produce operations that pass isOperation type guard', async () => {
    const operations = await parser.parseOperations(awdApiPath);
    for (const op of operations) {
      expect(isOperation(op), `PHP operation "${op.name}" should pass isOperation`).toBe(true);
    }
  });
});
...
describe.skipIf(!SDK_AVAILABLE)('PythonParser integration - YAML rate limit lookup', () => {
  const parser = new PythonParser(REPO_PATH);
  const awdApiPath = path.join(REPO_PATH, 'python/sdk/spapi/api/awd_v2024_05_09/awd_api.py');
  const applicationsApiPath = path.join(REPO_PATH, 'python/sdk/spapi/api/applications_v2023_11_30/applications_api.py');
  ...
  it('should produce operations that pass isOperation type guard (including null rateLimits)', async () => {
    ...
    expect(operations.length).toBeGreaterThan(0);
    for (const op of operations) {
      expect(isOperation(op), `Python operation "${op.name}" should pass isOperation`).toBe(true);
    }
  });
});
...
describe.skipIf(!SDK_AVAILABLE)('Cross-parser: missing YAML keys produce rateLimit: null', () => {
  /**
  ...
  it('operations not in YAML should have rateLimit: null and still pass isOperation', async () => {
    ...
    for (const op of nullRateLimitOps) {
      expect(isOperation(op), `Operation "${op.name}" with null rateLimit should pass isOperation`).toBe(true);
    }
    ...
    for (const op of withRateLimitOps) {
      expect(isOperation(op), `Operation "${op.name}" with rateLimit should pass isOperation`).toBe(true);
    }
  });
});
