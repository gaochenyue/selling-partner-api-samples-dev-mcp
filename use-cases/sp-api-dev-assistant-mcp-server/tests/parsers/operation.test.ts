...
import { isRateLimit, validateRateLimit } from '../../src/tools/code-generation-tools/models/operation.js';
...
const positiveNumber = fc.double({ min: 0.001, max: 1e6, noNaN: true, noDefaultInfinity: true });
  ...
  describe('Property 1: Type guard accepts extended RateLimit objects', () => {
    it('should accept RateLimit with valid base fields and optional valid burst/intervalInSeconds', () => {
      const rateLimitArb = fc
        ...
        .filter((obj) => obj.requestsPerSecond !== undefined || obj.requestsPerMinute !== undefined);
    ...
    it('should reject objects with invalid intervalInSeconds values', () => {
      ...
      fc.assert(
        fc.property(invalidIntervalArb, (obj) => {
          const errors = validateRateLimit(obj);
          expect(errors.length).toBeGreaterThan(0);
          expect(errors.some((e: string) => e.includes('intervalInSeconds'))).toBe(true);
        }),
        { numRuns: 200 },
      );
    });
