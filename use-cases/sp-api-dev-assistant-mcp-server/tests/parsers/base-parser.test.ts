...
import { BaseParser, RateLimitEntry } from '../../src/tools/code-generation-tools/parsers/base-parser.js';
import { Category, Operation, Model, RateLimit } from '../../src/tools/code-generation-tools/models/index.js';
...
class TestParser extends BaseParser {
  ...
  public testLookupRateLimit(className: string, methodName: string): Promise<RateLimit | null> {
...
const positiveNum = fc.double({ min: 0.001, max: 1e4, noNaN: true, noDefaultInfinity: true });

/** Arbitrary for alphanumeric non-empty strings (class/method names) */
const alphanumStr = fc.string({ minLength: 1, maxLength: 20 }).filter((s) => /^[a-zA-Z0-9]+$/.test(s));
...
function normalizeMethodName(methodName: string): string {
  const camel = methodName.replace(/_([a-z])/g, (_, c: string) => c.toUpperCase());
...
describe('BaseParser - loadRateLimitMap', () => {
  ...
  it('should cache result on second call (file read only once)', async () => {
    ...
    const fileExistsSpy = vi.spyOn(FileSystemUtils, 'fileExists').mockResolvedValue(true);
    const readFileSpy = vi.spyOn(FileSystemUtils, 'readFile').mockResolvedValue(yamlContent);
  ...
  describe('Property 3: YAML entry parsing round-trip', () => {
    it('should correctly parse 2-element entries with default interval of 1', async () => {
      await fc.assert(
        fc.asyncProperty(alphanumStr, positiveNum, positiveNum, async (key, rate, burst) => {
    ...
    it('should correctly parse 3-element entries with explicit interval', async () => {
      await fc.assert(
        fc.asyncProperty(alphanumStr, positiveNum, positiveNum, positiveNum, async (key, rate, burst, interval) => {
          const parser = new TestParser('/test/repo');
          const yamlObj: Record<string, number[]> = {};
    ...
    it('should skip malformed entries and preserve valid ones', async () => {
      const validEntryArb = fc.tuple(
        ...
        fc.oneof(
          ...
          fc.tuple(positiveNum, positiveNum, positiveNum).map((arr) => arr as number[]),
        ),
      );
...
describe('BaseParser - lookupRateLimit', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (className, methodName, rate, burst, interval) => {
            ...
            const result = await parser.testLookupRateLimit(className, methodName);
  ...
  describe('Property 6: Missing key lookup returns null', () => {
    it('should return null for keys not in the map', async () => {
      await fc.assert(
        fc.asyncProperty(alphanumStr, alphanumStr, async (className, methodName) => {
          const parser = new TestParser('/test/repo');
