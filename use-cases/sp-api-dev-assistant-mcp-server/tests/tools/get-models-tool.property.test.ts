...
describe('GetModels Property Tests', () => {
  ...
  describe('Property 6: Models Backward Compatibility', () => {
    const modelArbitrary = fc.record({
      ...
      swaggerType: fc.constantFrom('object', 'string', 'integer', 'boolean', 'array', 'number'),
    ...
    it('should include all fields when included_data is not specified', () => {
      fc.assert(
        fc.asyncProperty(fc.array(modelArbitrary, { minLength: 1, maxLength: 10 }), async (models) => {
          // Create fresh tool and mock for this iteration
          const freshTool = new GetModels();
          const freshMock = (freshTool as any).discoveryService;
    ...
    it('should return all models when models filter is not specified', () => {
      fc.assert(
        fc.asyncProperty(fc.array(modelArbitrary, { minLength: 0, maxLength: 15 }), async (models) => {
          // Create fresh tool and mock for this iteration
          const freshTool = new GetModels();
          const freshMock = (freshTool as any).discoveryService;
      ...
      fc.assert(
        fc.asyncProperty(fc.array(modelArbitrary, { minLength: 1, maxLength: 10 }), async (models) => {
          ...
          parsedResult.items.forEach((model: any, index: number) => {
            ...
            expect(model.enumValues === null || Array.isArray(model.enumValues)).toBe(true);
  ...
  describe('Property 2: Models Name Filtering', () => {
      ...
      fc.assert(
        fc.asyncProperty(
          ...
          async (models, filterNames) => {
            ...
            const expectedFiltered = models.filter((m) => lowerFilterNames.includes(m.name.toLowerCase()));
            ...
            parsedResult.items.forEach((model: any) => {
              const matches = lowerFilterNames.includes(model.name.toLowerCase());
    ...
    it('should handle multiple comma-separated model names', () => {
      fc.assert(
        fc.asyncProperty(fc.array(modelArbitrary, { minLength: 3, maxLength: 20 }), async (models) => {
          // Create fresh tool and mock for this iteration
          const freshTool = new GetModels();
          const freshMock = (freshTool as any).discoveryService;
  ...
  describe('Property 4: Models Field Projection', () => {
    const validFields = ['name', 'swaggerType', 'attributeMap', 'isEnum', 'enumValues'];
