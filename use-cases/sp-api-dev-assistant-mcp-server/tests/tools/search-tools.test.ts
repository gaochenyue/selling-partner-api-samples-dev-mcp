...
function createManager(overrides?: Partial<IndexManagerConfig>): IndexManager {
  return new IndexManager({ ...defaultConfig, ...overrides }, stubVectorStore, stubEmbeddingService);
}
...
describe('IndexManager', () => {
  describe('stripHtml', () => {
    ...
    it('should handle nested elements', () => {
      const manager = createManager();
      const html = '<div><p>Outer <span>inner <strong>bold</strong></span> text</p></div>';
    ...
    it('should preserve links as "text (url)" format', () => {
      const manager = createManager();
      const html = '<p>Refer to <a href="https://example.com/docs">the docs</a> for details.</p>';
      const result = manager.stripHtml(html);
    ...
    it('should convert HTML tables to natural language using column headers', () => {
      ...
      expect(result).toContain('Operation: getOrders, Rate: 0.0167, Burst: 20.');
    });
    ...
    it('should handle tables with links in cells', () => {
      ...
      `;
      const result = manager.stripHtml(html);
      expect(result).toContain('Operation: getOrder (https://example.com/api), Rate: 0.5.');
    });
  });
        ...
        fc.assert(
          fc.property(
            ...
            (text, maxTokens, overlapFraction) => {
              ...
              const overlapTokens = Math.floor((overlapFraction / 100) * (maxTokens - 1));
...
import type { IndexMetadata, RawDocument, ScoredItem } from '../../src/tools/search-tools/types';
    ...
    fc.assert(
      fc.property(
        ...
        (meta, bodyText) => {
          ...
          const text = manager.stripHtml(`<div><h1>${meta.title}</h1><p>${bodyText}</p></div>`);
...
describe('VectorStore search behavior (via SearchTool)', () => {
  ...
  it('search should return at most top_k results', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 20 }), fc.integer({ min: 0, max: 30 }), async (topK, indexSize) => {
        ...
        const searchTool = new SearchTool(stubEmbeddingService, mockVectorStore, createManager());
        const result = await searchTool.search({ query: 'test', top_k: topK });

        if (indexSize === 0) {
          expect(result.isError).toBeFalsy();
        } else {
          expect(result.isError).toBeFalsy();
          const resultMatches = result.content[0].text.match(/Result \d+/g) || [];
          expect(resultMatches.length).toBeLessThanOrEqual(Math.min(topK, indexSize));
        }
      }),
      { numRuns: 50 },
    );
...
describe('SearchTool formatting', () => {
  function makeSearchTool(items: ScoredItem[]): SearchTool {
    ...
    return new SearchTool(stubEmbeddingService, mockVectorStore, createManager());
  }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (docs) => {
          ...
          const result = await tool.search({ query: 'test', top_k: docs.length });
  ...
  it('formatted results should include Source link for each result', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(fc.webUrl(), { minLength: 1, maxLength: 3 }), async (urls) => {
        ...
        const tool = makeSearchTool(items);
        const result = await tool.search({ query: 'test', top_k: urls.length });
        const text = result.content[0].text;
...
describe('SearchTool unit tests', () => {
  it('should return informational message when index is empty', async () => {
    ...
    const tool = new SearchTool(stubEmbeddingService, emptyVectorStore, createManager());
  ...
  it('should return error response when embedding fails', async () => {
    ...
    const tool = new SearchTool(failingEmbedding, hasIndexStore, createManager());
  ...
  it('should return no results message when search returns empty', async () => {
    ...
    const tool = new SearchTool(stubEmbeddingService, emptySearchStore, createManager());
  ...
  it('should format results with correct structure', async () => {
    ...
    const tool = new SearchTool(stubEmbeddingService, mockStore, createManager());
...
describe('IndexManager lifecycle', () => {
  it('should register and retrieve crawlers', () => {
    ...
    const mockCrawler = {
      name: 'test-crawler',
      crawl: async () => [],
      getSourceMetadata: () => ({ name: 'test', baseUrl: 'https://test.com', supportedLocales: ['en-US'] }),
    };
    // Should not throw
