...
const nonPaginationActionArgsMap: Record<string, fc.Arbitrary<Record<string, any>>> = {
  ...
  clone_repo: fc.record(
    { repositoryUrl: fc.constant('https://example.com/repo'), targetPath: nonEmptyString },
    { requiredKeys: [] },
  ),
};
...
const nonPaginationActionWithArgs = fc
  ...
  .chain((action) => nonPaginationActionArgsMap[action].map((args) => ({ action, args })));
...
const paginationActionWithIncludedData = fc.oneof(
  operationsWithIncludedData.map((args) => ({ action: 'get_operations' as const, args })),
  modelsWithIncludedData.map((args) => ({ action: 'get_models' as const, args })),
);
...
describe('Bug Condition Exploration: Leaked Defaults and camelCase Mismatch', () => {
  ...
  it('Property 1a: non-pagination actions should NOT receive leaked page/pageSize defaults', async () => {
    await fc.assert(
      fc.asyncProperty(nonPaginationActionWithArgs, async ({ action, args }) => {
  ...
  it('Property 1b: get_operations/get_models should receive included_data (snake_case), not includedData (camelCase)', async () => {
    await fc.assert(
      fc.asyncProperty(paginationActionWithIncludedData, async ({ action, args }) => {
        const { masterTool, toolMap } = createMasterTool();
...
const anyValidActionWithArgs = fc
  ...
  .chain((action) => allActionArgsMap[action].map((args) => ({ action, args })));

// Arbitrary for unknown action strings (not in valid action set)
const unknownActionArbitrary = fc
  .string({ minLength: 1, maxLength: 50 })
  .filter((s) => s.trim().length > 0 && !(validActions as readonly string[]).includes(s));
...
const paginationActionWithExplicitPagination = fc.oneof(
  operationsWithExplicitPagination.map((args) => ({ action: 'get_operations' as const, args })),
  modelsWithExplicitPagination.map((args) => ({ action: 'get_models' as const, args })),
);
...
describe('Preservation: Routing, Error Handling, and Parameter Forwarding', () => {
  ...
  it('Property 2c: explicit page/pageSize are forwarded to pagination sub-tools', async () => {
    await fc.assert(
      fc.asyncProperty(paginationActionWithExplicitPagination, async ({ action, args }) => {
        const { masterTool, toolMap } = createMasterTool();
